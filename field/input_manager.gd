...
@onready var player_manager: PlayerBattleManager = $"../PlayerBattleManager"
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_RIGHT and event.pressed:
		...
		#
		if not turn_manager.is_player_turn() or turn_manager.current_turn == 1: return
		...
		if card_slot is CardSlot and card_slot.card_in_slot:
			...
			if card in player_manager.attacking_card_this_turn: return
			
			context_menu.set_card(card)
